...
int OpenServerCommand::insertingBufferToMap(char *buffer, int buffer_len) {
    std::unique_lock<std::mutex> lck(mtx_open_server);
    int array_index = 0;
    ...
    m_reading_from_sim = false;
    cv_open_server.notify_all();
    return 0;
}
...
void OpenServerCommand::closeConnection() {
    close(m_fd);
    m_connected = false;
}

bool OpenServerCommand::isReadingFromSim() {
    return m_reading_from_sim;
}

bool OpenServerCommand::isNotReadingFromSim() {
    return !m_reading_from_sim;
}
